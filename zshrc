...
PROMPT=$'%{$fg_bold[green]%}%n %{$fg[blue]%}%D{[%X]} %{$reset_color%}%{$fg[white]%}%~%{$reset_color%} $(git_prompt_info) %{$fg[yellow]%}$(virtualenv_prompt_info)\
%(?:%{$fg[green]%}➜ %{$fg_bold[green]%} %#:%{$fg[red]%}➜ %{$fg_bold[red]%} %#)%{$reset_color%} '
...
alias 'gcn!'='git commit --amend --verbose --no-edit --date=now'
alias ggpushf='git push --force origin "$(git_current_branch)"'
alias gmd='git merge origin/"$(git_develop_branch)"'
